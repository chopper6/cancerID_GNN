...
def run_practice():
	...
	untrained_err = pnet.evaluate(x=Xtest, y=Ytest, verbose=0)
	history = pnet.fit(
		Xtrain,
		Ytrain,
		batch_size=64,
...
class PracticeLayer(keras.layers.Layer):
	...
	def build(self, input_shape):
		...
		print('\n\nbuild:',input_shape[-1],input_shape[0],input_shape[1], num_edges,'\n\n')
		#assert(0) # instead #ws should be num_edges + num_nodes (curr is num_edges*num_nodes + num_nodes)
		...
		self.targets = self.adj.indices[0,:] # indices of target nodes for each edge
		print('sources/targets=',self.sources.shape, self.targets.shape)
		...
		#self.weighted_ngh_holder = tf.sparse.SparseTensor(ind,self.adj.values*edges_values,self.adj.dense_shape)

	
	#def call(self, inputs):
	#	return tf.matmul(inputs, self.w) 

	def junk_attemps(self, all_inputs):
		...
		#num_edges = len(self.w)
		
		#if num_instances != None:
		#	wnghs_indices = tf.reshape(tf.tile(self.adj.indices,num_instances), (num_instances,num_edges))
		#else:
		#	wnghs_indices = self.adj.indices
		#	self.adj.indices[0]=self.adj.indices
		#assert(wnghs_indices[0]==wnghs_indices[1])
		#print('prev dense shape=',self.adj.dense_shape)
		#assert(0)
		#ind = self.adj.indices
		#ind= ind[None,:]
		#print(self.adj.indices.shape)
		#weighted_nghs=tf.sparse.SparseTensor(ind,self.adj.values*edges_values,self.adj.dense_shape)
